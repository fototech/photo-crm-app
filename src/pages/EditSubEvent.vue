<template>
  <q-page class="q-pa-md bg-grey-2">
    <div class="text-h6 q-mb-md">✏️ Edit Sub Event</div>
      ...
      <q-card flat class="q-pa-md bg-white">
        ...
        <div class="q-mt-lg">
          ...
          <div v-for="(row, i) in teamAssignments" :key="i" class="row q-col-gutter-md q-mb-sm items-center">
            <q-select filled v-model="row.team_id" :options="teams" option-label="team_name" option-value="id" emit-value map-options label="Team" dense class="col-md-2 col-12" @update:model-value="() => loadTeamRelatedData(row)" />
            <q-select filled v-model="row.sub_category_id" :options="row.subCategories" option-label="name" option-value="id" emit-value map-options label="Sub Category" dense class="col-md-2 col-12" />
            <q-select filled v-model="row.member_id" :options="row.members" option-label="name" option-value="id" emit-value map-options label="Team Member" dense class="col-md-2 col-12" />
        ...
        <div class="q-mt-lg">
          ...
          <div v-for="(row, i) in vendorAssignments" :key="i" class="row q-col-gutter-md q-mb-sm items-center">
            <q-select filled v-model="row.vendor_id" :options="vendors" option-label="name" option-value="id" emit-value map-options label="Vendor" dense class="col-md-2 col-12" @update:model-value="() => loadVendorRelatedData(row)" />
            <q-select filled v-model="row.vendor_sub_category_id" :options="row.subCategories" option-label="name" option-value="id" emit-value map-options label="Sub Category" dense class="col-md-2 col-12" />
            <q-select filled v-model="row.vendor_member_id" :options="row.members" option-label="name" option-value="id" emit-value map-options label="Vendor Member" dense class="col-md-2 col-12" />
...
const subEventId = route.params.sub_event_id
...
const loadData = async () => {
  try {
    const res = await axios.get(`/edit-sub-assignment/${eventId}/${subEventId}`, {
      params: { user_id }
    })

    const data = res.data
    form.value = {
      sub_event_name: data.sub_event.sub_event_name,
      description: data.sub_event.description,
      event_date: data.sub_event.event_date,
      start_time: data.sub_event.start_time,
      end_time: data.sub_event.end_time,
      sub_event_venue_name: data.sub_event.sub_event_venue_name,
      sub_event_venue_address: data.sub_event.sub_event_venue_address,
      contact_person_name: data.sub_event.contact_person_name,
      contact_person_mobile: data.sub_event.contact_person_mobile,
      google_map_link: data.sub_event.google_map_link
    }

    teams.value = data.teams
    vendors.value = data.vendors

    teamAssignments.value = data.assigned_teams.map(t => ({
      team_id: Number(t.team_id),
      sub_category_id: Number(t.sub_category_id),
      member_id: Number(t.member_id),
      team_note: t.team_note || '',
      subCategories: data.team_sub_categories.filter(s => s.team_id === t.team_id).map(s => ({ ...s, id: Number(s.id) })),
      members: data.team_members.filter(m => m.team_category_id === t.team_id).map(m => ({ ...m, id: Number(m.id) }))
    }))

    vendorAssignments.value = data.assigned_vendors.map(v => ({
      vendor_id: Number(v.vendor_id),
      vendor_sub_category_id: Number(v.vendor_sub_category_id),
      vendor_member_id: Number(v.event_vendor_id),
      vendor_note: v.vendor_note || '',
      subCategories: data.vendor_sub_categories.filter(s => s.vendor_id === v.vendor_id).map(s => ({ ...s, id: Number(s.id) })),
      members: data.vendor_members.filter(m => m.vendor_sub_category_id === v.vendor_sub_category_id).map(m => ({ ...m, id: Number(m.id) }))
    }))
  } catch (err) {
...
const submitForm = async () => {
  try {
    const payload = {
      ...
      team_ids: teamAssignments.value.map(t => t.team_id),
      team_sub_category_ids: teamAssignments.value.map(t => t.sub_category_id),
      member_ids: teamAssignments.value.map(t => t.member_id),
      team_notes: teamAssignments.value.map(t => t.team_note),
      vendor_ids: vendorAssignments.value.map(v => v.vendor_id),
      vendor_sub_category_ids: vendorAssignments.value.map(v => v.vendor_sub_category_id),
      vendor_member_ids: vendorAssignments.value.map(v => v.vendor_member_id),
      vendor_notes: vendorAssignments.value.map(v => v.vendor_note)
    }

    const res = await axios.post(`/update-sub-event/${eventId}/${subEventId}`, payload)
    if (res.data?.success) {
      $q.notify({ type: 'positive', message: res.data.message || 'Updated successfully!' })
      router.push(`/listingSubEvent/${res.data.event_id}`)
    } else {
      $q.notify({ type: 'negative', message: res.data.message || 'Update failed!' })
    }
  } catch (err) {
    console.error('Form submission error:', err)
    $q.notify({ type: 'negative', message: 'Server error during update!' })
  }
}
